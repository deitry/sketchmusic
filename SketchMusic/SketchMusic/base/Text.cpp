...
void SketchMusic::Text::deleteSymbol(Cursor^ cur, ISymbol^ sym)
{
	...
	for (;lookup != max; lookup++)
	{
		...
		{
			// проверка на то, что это действительно тот символ - помимо типа провер€ем на значение
			switch (type)
			{
			...
			default:
				// по умолчанию на тип не смотрим, полага€, что там только один такой символ
				_t.erase(lookup);
				return;
			}
		}
...
IJsonValue^ SketchMusic::Text::serialize()
{
	...
	{
		// не вызываем сериализацию дл€ каждого элемента в отдельности, чтобы немного сэкономить на объектах.
